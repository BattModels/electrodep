...
#outputs = exodus
  ...
  [./ButlerVolmer]
  type = DerivativeParsedMaterial
#  function = 'Ls * if(cl<0, if(h<1, sqrt(-cl * (1.0-h) / stdconc), 0), 0) * (exp(pot * Fconst/RT /2.0) - exp(-pot * Fconst/RT /2.)) * dh'
  ...
  [./BVredn]
  type = DerivativeParsedMaterial
  function = 'Ls * cl * (1-h) / stdconc * exp(-pot * Fconst / RT / 2.) * dh'
  args = 'pot w eta'
  f_name = Gred
  derivative_order = 1
  material_property_names = 'Ls dh:=D[h(eta),eta] h(eta) cl:=D[f1(w),w] RT Fconst stdconc'
  ...
  [./BVox]
  type = DerivativeParsedMaterial
  function = 'Ls * exp(pot * Fconst / RT / 2.) * dh'
  args = 'pot w eta'
  f_name = Gox
  derivative_order = 1
