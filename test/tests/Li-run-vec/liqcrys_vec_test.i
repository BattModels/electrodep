...
[Kernels]
  active = 'nlc_diff nlc_surf nlc_mag lagrange w_dot Diffusion1 Diffusion2 poteqn coupled_pot BVnlc coupled_etadot AC_bulk AC_int AC_bulk_lc_penalty e_dot'
  [./nlc_diff]
...
#outputs = exodus
  ...
  #   outputs = exodus
  [../]
  [./ButlerVolmer_spelled]
  type = DerivativeParsedMaterial
  function = 'Ls * (exp(pot * Fconst/RT/2.0) - exp((w-el)/RT)/( 1.0+exp((w-el)/RT) ) * (1.0 - eta*eta*eta*(6.0*eta*eta - 15.0*eta + 10.0)) / stdconc * exp(-pot * Fconst/RT/2.0)) * (30.0*eta*eta*eta*eta -60.0*eta*eta*eta +30.0*eta*eta)'
  args = 'pot w eta'
  f_name = GG
  derivative_order = 2
  material_property_names = 'Ls RT Fconst stdconc el'
#  outputs = other
  [../]
  [./ButlerVolmer]
  type = DerivativeParsedMaterial
