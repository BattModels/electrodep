...
{
  Real gradderiv =  -2.0 * (_n[_qp] * _gradeta[_qp]) * 0.0 - 2.0 * _n[_qp] * (_grad_n[_qp] * _gradeta[_qp] + _grad_gradeta[_qp] * _n[_qp]); //There is a divergence in 1st term, not implemented yet
...
KineticsLiqCrysVar::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  if (jvar == _n_id)
    {
      Real pgradderivpn = -2.0 * (_n_phi[_j][_qp] * _gradeta[_qp]) * 0.0 - 2.0 * _n_phi[_j][_qp] * (_grad_n[_qp] * _gradeta[_qp] + _grad_gradeta[_qp] * _n[_qp]) - 2.0 * _n[_qp] * ( _n_grad_phi[_j][_qp] * _gradeta[_qp] + _grad_gradeta[_qp] * _n_phi[_j][_qp]);
    ...
    {
      Real pgradderivpgradeta = -2.0 * (_n[_qp] * _gradeta_phi[_j][_qp]) * 0.0 - 2.0 * _n[_qp] * (_grad_n[_qp] * _gradeta_phi[_j][_qp] + _gradeta_grad_phi[_j][_qp] * _n[_qp]);
